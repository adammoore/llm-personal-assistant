...
- **Phone capture via iMessage (reliable, no Signal fight)**: `lib/imessage.py` reads the local `chat.db` (read-only, immutable; `text` + best-effort `attributedBody` decode). The monitor now watches for self-sent messages prefixed **todo/pa/capture/task …** and turns them into tasks (rebuilds the dashboard + a ✓ Signal confirmation). Baseline seeded so history isn't captured. Same reader will feed the unified inbox next.
- **Person-as-a-thread (ZigZag payoff)**: click any person (People card rows or the 'everyone' chips) → the whole view filters to items involving them — their mail, plus tasks/meetings that mention them — with a '👤 Name — everything involving them · clear' banner. The person dimension, followed across sources.
- **Local people layer (ADR-004 A1)**: Docker/Colima wouldn't provision, so shipped the server-free path — `lib/people.py` (git-ignored `data/people.json`), **auto-seeded from real mail contacts** (noise-filtered, two-part names). People card now shows birthdays + 'not heard from' reconnect nudges + a ↻ sync button (`/sync-people`). `lib/monica.py` stays ready for whenever Monica is self-hosted.
